...
import { ChevronRight, FileText, GraduationCap, Library, Search } from 'lucide-react';
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-primary p-2.5 rounded-2xl text-white shadow-lg shadow-blue-100">
          ...
          <form onSubmit={handleSearch} className="flex-1 max-w-2xl mx-12">
            <div className="relative group">
              ...
              />
              <button type="submit" className="absolute right-3 top-2 px-4 py-1.5 bg-primary text-white rounded-xl text-xs font-bold shadow-md hover:bg-primary-dark transition-all">
                搜索
              </button>
            </div>
          </form>
          ...
          <div className="flex items-center gap-4 border-l border-slate-100 pl-6">
            <button
              ...
              className="text-slate-400 font-bold text-xs uppercase hover:text-red-500 transition-colors"
            >
              退出登录
            </button>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1 font-medium">当前可访问 {textbooks.length} 本教材</p>
          </div>
          {isSearching && (
            <button onClick={clearSearch} className="text-primary font-bold text-sm hover:underline">
              返回全部教材
            </button>
          )}
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 text-slate-300 gap-4">
            ...
            <p className="font-bold uppercase text-xs tracking-widest">正在检索知识库...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {textbooks.map((textbook: any) => (
              <div
                key={textbook.id}
                onClick={() => navigate(`/resource/textbook/${textbook.id}`)}
                className="bg-white rounded-[2rem] border border-slate-200 p-2 hover:shadow-2xl hover:shadow-blue-900/10 transition-all group cursor-pointer hover:-translate-y-1"
              >
                <div className="aspect-[3/4] bg-slate-50 rounded-[1.5rem] mb-4 flex items-center justify-center relative overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4">
                    <span className="bg-white/80 backdrop-blur px-3 py-1 rounded-full text-[10px] font-black text-primary border border-primary/10 uppercase shadow-sm">
                      PDF
                    </span>
                  </div>
                </div>
                <div className="px-4 pb-6">
                  <h3 className="text-lg font-black text-slate-800 mb-1 group-hover:text-primary transition-colors line-clamp-1">{textbook.title}</h3>
                  <div className="flex items-center gap-2 mb-6">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-tight">{textbook.author || '未知作者'}</span>
                    <span className="w-1 h-1 bg-slate-200 rounded-full" />
                    <span className="text-[10px] font-bold text-slate-300 tracking-tighter">ISBN: {textbook.isbn || '--'}</span>
                  </div>
                  <div className="flex items-center justify-end pt-4 border-t border-slate-50">
                    <div className="bg-slate-50 group-hover:bg-primary p-2 rounded-xl transition-all">
        ...
        {!loading && textbooks.length === 0 && (
          <div className="text-center py-32 bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
            ...
            <button onClick={clearSearch} className="mt-8 text-primary font-black text-sm uppercase tracking-widest hover:underline">
              清除搜索条件
            </button>
          </div>
        )}
